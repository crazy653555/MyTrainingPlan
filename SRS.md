...
本產品是一個獨立運作的純前端應用程式，不需要後端伺服器與資料庫支援。所有的狀態與資料（如訓練清單配置、全域休息影片網址）都將利用瀏覽器的 Local Storage 或 IndexedDB 技術進行持久化儲存。影片播放功能則是高度依賴外部的 YouTube IFrame API (透過 `react-youtube` 套件實作) 以及網頁原生的 HTML5 `<video>` API (用於播放本地端檔案)。
...
- **功能列表**：
    - **階段列表顯示**：以清單方式呈現所有訓練階段，包含該階段的名稱、影片來源(YouTube 或 Local)、運動時間、休息時間以及影片截取時段。
    - **階段新增與編輯**：
...
- **實作方式與限制 (本地影片)**：
    - 基本訓練清單使用瀏覽器 Local Storage 保存。
    - **＊技術問題與限制 (本地檔案)**：為了安全性，瀏覽器不允許網頁直接以絕對路徑 (`C:\...`) 讀取使用者電腦檔案。若使用者選擇本地檔案，程式會取得一個臨時的 URL 物件 (`ObjectURL`) 或檔案指標 (`File` object)。這個指標在網頁**重新整理**後就會失效。
    - **解決方案評估**：
      1. 若要完美持久化，需使用 **File System Access API (IndexedDB + 檔案句柄)**，但需支援度考量且重整後可能還需跟使用者請求閱讀權限。
      2. 或者採用較簡易方式：將清單保留，但若偵測到重整遺失了本地檔案參考，會在介面上顯示「影片已斷裂，請重新選擇檔案」的提示。
