...
- **功能列表**：
    ...
    - **階段新增與編輯**：
        ...
        - 影片片段設定：開始播放時間 (Start Time)、結束播放時間 (End Time)；具備防呆機制，若勾選「End (Play to end)」則直接播放至結尾；若手動設定，系統會自動連動檢核並往後順調，確保結束秒數必然大於起播秒數。
    - **操作排列與刪除**：使用者可以將特定階段上移、下移或刪除。刪除操作前必須顯示自訂樣式的全螢幕警告視窗 Modal，要求使用者確認，防範誤觸。
    - **全域休息影片 (Global Rest Video)**：使用者可全域指定一支 YouTube 影片做為背景預覽，即時修改儲存。
...
- **功能列表**：
    ...
    - **播放器控制與全螢幕擴展**：
        - 暫停/繼續功能、跳過功能、全螢幕切換與退出。
        - **全螢幕模式下的擴展顯示**：為彌補側邊欄在全螢幕時隱藏，畫面需浮現提示，預告「下一個訓練項目」的資訊。
